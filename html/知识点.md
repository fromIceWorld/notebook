...
defer: 只会在dom解析完,DOMContentLoaded 事件触发之前完成
```

#### 操作DOM慢

```typescript
1.DOM属于渲染引擎,JS属于JS引擎,操作DOM涉及两个线程之间通信,损耗性能,并且操作DOM还会带来重绘回流操作。
...
```typescript
当初始的 HTML 文档被完全加载和解析完成之后，DOMContentLoaded 事件被触发，而无需等待css、img和子框架的完全加载。
