...
`脱离文本流方法`：position：absolute,fixed
```
...
```typescript
float 使元素脱离文档流，漂浮于文档流的上方，但不脱离文本流【文档不认可[认为它不存在]，但文本认可float占据的位置】。
...
效果类似于文本环绕。
`浮动会脱离普通文档流，漂浮于文档流的上方，但不会脱离文本流`
```

### 绝对定位

```
绝对定位会使元素脱离普通流也会脱离文本流，因此绝对定位元素不会对其兄弟元素造成影响，元素具体的位置由绝对定位的坐标和
非 static 元素的父级元素决定。
```
...
`更改`
document.documentElement.style.setProperty('--red-them','green')
