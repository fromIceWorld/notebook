...
while (index < source.length) {
    for (let pre = index - 1; pre >= 0; pre--) {
        if (source[pre] > source[pre + 1]) {
            [source[pre],source[pre + 1]] = [source[pre + 1],source[pre]]
        } else {
            break;
        }
    }
...
for(let i = 0, ;i < resource.length;i++){
    ...
    for(let from = i;from < resource.length; from++){
...
function Initheap(){
    for(let len = Math.floor(end/2)-1; len>=0; len--){
        fix(len)
    } 
}
function fix(n){
    let cache,left = 2*n+1,right = z*n+2;
    if(source[n] < source[left] || source[n] < source[right]){
        if(source[right] >= source[left]){
             cache = source[n];
             source[n] = source[right];
             source[right] = cache;
            if(right <= Math.floor(end/2)-1){
                fix(right)
            }
        }else{
            cache = source[n];
            source[n] = source[left];
            source[left] = cache;
            if(left <= Math.floor(end/2)-1){
                fix(left)
            }
        }
    }
...
for(let from = 0,end = resource.length-1; from < end; end-- ){
    for(let i = 0; i< end;i++){
        if(resource[i] > resource[i+1]){
            [resource[i],resource[i+1]] = [resource[i+1],resource[i]]
        }
	}
}
稳定性：稳定
