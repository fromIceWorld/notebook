...
**程序是一堆视图(View)组成的，而每一个视图又是由不同类型节点组成的。**
...
View的属性可以更改，但是视图中元素的结构(数量和顺序)不能更改，想要更改只能通过`ViewContainerRef`来插入，移动或者移除嵌入的视图
...
```typescript
@params flags: ViewFlags,                       //视图标记：None、OnPush
...
```
保证当前执行的变化检测中，不会有变化发生,如果发现绑定发生了变化或者 DOM 需要更新，就抛出异常
```
